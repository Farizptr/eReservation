...
const ManageOrderScreen = () => {
  ...
  const handleApprove = async (orderId) => {
    ...
    try {
      ...
      if (orderData && orderData.modifiedOrder) {
        const { modifiedOrder } = orderData;

        const newOrderDataArray = Object.keys(modifiedOrder)
          .filter((key) => !isNaN(key)) // Filter out non-numeric keys
          .map((key) => {
            return {
              keterangan: modifiedOrder[key].keterangan || "",
              nama_barang: modifiedOrder[key].nama_barang || "",
              quantity: modifiedOrder[key].quantity || 0,
              satuan: modifiedOrder[key].satuan || "",
              date: modifiedOrder.date || "",
            };
          });

        for (const newOrderData of newOrderDataArray) {
          await addDoc(collection(db, "data_pemesanan"), newOrderData);
        }
      }
